...
--
-- Versao expandida (base: gabarito oficial do marco-06), espelhando as 3
-- constraints novas do 02_vra_auditado.sql.
-- ---------------------------------------------------------------------------
...
SELECT
  *,
  concat_ws(' | ',
    ...
    CASE WHEN NOT codigo_di_catalogado THEN 'codigo_di_catalogado_na_anac' END,
    CASE WHEN NOT codigo_tipo_linha_catalogado THEN 'codigo_tipo_linha_catalogado_na_anac' END,
    CASE WHEN linha_duplicada THEN 'linha_nao_duplicada' END
...
WHERE NOT (
  ...
  AND codigo_di_catalogado
  AND codigo_tipo_linha_catalogado
  AND NOT linha_duplicada
);
